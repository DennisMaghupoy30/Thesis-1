        ...
        if (!isWindows) {
            ...
            return new Promise(async (resolve) => {
                ...
                v4l2Process.on('close', async () => {
                    ...
                    // Now detect resolution for each real camera
                    for (let i = 0; i < cameras.length; i++) {
                        const camera = cameras[i];
                        const device = camera.devices[0];
                        const resolution = await detectCameraResolution(device);
...
async function main() {
    ...
    cameras.forEach(camera => {
        ...
        console.log(`  Camera ${camera.id}: ${displayName} -> ${camera.device} -> Port ${camera.port} (${camera.resolution} @ ${camera.fps}fps)`);
